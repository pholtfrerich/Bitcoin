...
"""TxFetcher Class
    Description: looks up all of the transactions referenced by the transaction input id, verifies how many sats in UXTOs the sender has to spend
    Attributes (init): None
    Dunders: None
    Methods:
        get_url(testnet):
            inputs: testnet?
            outputs: returns url of block explorer
        fetch(tx_id, testnet, fresh):
            inputs: 
                tx_id: the transaction id being looked up
                testnet?
                fresh: no idea
            outputs: returns the transaction object being looked up
        load_cache(filename)
        dump_cache(filename)
...
"""Tx Class
    Description: Defines an object of a Bitcoin transaction 
    Attributes (init):
        inputs: 
            version: version
            inputs: tx_ins
    ...
    Methods:
        id():
            outputs: returns human-readable hexadecimal of the transaction hash
                the id() is what block explorers use to look up transactions
        hash():
            outputs: returns the binary hash in bytes of the serializatoin of this transaction object, in little endian
        parse(cls,s,testnet): a class method as it returns a new instance of a Tx object
            inputs:
                s: a stream of bytes
                testnet?
                cls: does not use self since it returns an object of the class where the method resides, a Class Method
            outputs: returns a Tx object from the stream of bytes
        serialize():
            outputs: Returns the byte serialization of the transaction
        fee():
            inputs: testnet?
            outputs: returns the fee of the Tx in satoshis
"""
class Tx:
...
"""TxIn:
    Description: Defines the object of the input portion of an Bitcoin Tx
    Attributes (init):
        prev_tx: the id of a the previous transaction being referenced
        prev_index: the id/index of the output transaction being referenced, since it is possible for a previous Tx to contain more than one output
        script_sig: the signature of the private key owner allowing access to spend prevous Tx outputs
        sequence: not important?, orginally designed to be used as locktime for high frequency transactions
    Dunders:
        repr: replaces operator "print"
    Methods:
        parse(s):
            inputs: stream of Tx
            outpus: returns the TxIn object of the transaction input portion in a Tx
        serialize():
            outputs:
        fetch_tx(testnet)
            inputs: testnet?
            outputs: Returns the byte serialization of the transaction input
        value(testnet)
            inputs: testnet?
            outputs: returns the output value by looking up the tx hash. Returns the amount in satoshi
        script_pubkey(testnet)
            inputs: testnet?
            outputs: returns the ScriptPubKey by looking up the tx hash. Returns a Script object
"""
class TxIn:
...
"""TxOut:
    Description: Defines the object of the output portion of a Bitcoin Tx
    Attributes (init):
        amount: the amount of sats being sent
        script_pubkey: where the sats are being sent
    Dunders:
        repr: replaces operator "print"
    Methods:
        parse(s):
            inputs: stream of Tx
            outpus: returns the TxOut object of the transaction output portion in a Tx
        serialize():
            outputs: returns the byte serialization of a TxOut object
"""
class TxOut:
